...
function AmtStackCreateService(wsmanStack) {
    ...
    obj.RealmNames = "|| Umleitung || Hardware-Asset | Fernbedienung | Speicher | Event Manager | Speicheradministrator | Agentenpräsenz Lokal | Agentenpräsenz Remote | Leistungsschalter | Netzwerkzeit | Allgemeine Informationen | Firmware-Aktualisierung | EIT | LocalUN | Endpunktzugriffskontrolle | Endpunktzugriffskontrolle Admin | Ereignisprotokollleser | Überwachungsprotokoll | ACL-Bereich ||| Lokales System".split('|');
    obj.WatchdogCurrentStates = { 1: "Nicht angefangen", 2: "Gestoppt", 4: "Laufen", 8: "Abgelaufen", 16: "Suspendiert" };
    ...
    function _GetEventDetailStr(eventSensorType, eventOffset, eventDataField, entity) {
        if (eventSensorType == 15) {
            if (eventDataField[0] == 235) return "Ungültige Daten";
            if (eventOffset == 0) {
        ...
        if ((eventSensorType == 18) && (eventDataField[0] == 170)) { // System watchdog event
            return "Agent Watchdog" + char2hex(eventDataField[4]) + char2hex(eventDataField[3]) + char2hex(eventDataField[2]) + char2hex(eventDataField[1]) + '-' + char2hex(eventDataField[6]) + char2hex(eventDataField[5]) + '-...' + " gewechselt zu" + obj.WatchdogCurrentStates[eventDataField[7]];
        }

        if ((eventSensorType == 5) && (eventOffset == 0)) { // System chassis
            return "Fall Eindringen";
        }

        if ((eventSensorType == 192) && (eventOffset == 0) && (eventDataField[0] == 170) && (eventDataField[1] == 48))
        {
            if (eventDataField[2] == 0) return "Eine Remote-Serial-Over-LAN-Sitzung wurde eingerichtet.";
            if (eventDataField[2] == 1) return "Remote Serial Over LAN-Sitzung beendet. Die Benutzersteuerung wurde wiederhergestellt.";
            if (eventDataField[2] == 2) return "Eine Remote-IDE-Umleitungssitzung wurde eingerichtet.";
            if (eventDataField[2] == 3) return "Remote-IDE-Umleitungssitzung beendet. Die Benutzersteuerung wurde wiederhergestellt.";
        }
        ...
        if (eventSensorType == 36) {
            ...
            if (eventDataField[0] == 0xAA) nic = "verdrahtet"; // TODO: Add wireless *****
            //if (eventDataField[0] == 0xAA) nic = "wireless";

            if (handle == 4294967293) { return "Alle empfangenen Paketfilter wurden abgeglichen" + nic + " Schnittstelle."; }
            if (handle == 4294967292) { return "Alle ausgehenden Paketfilter wurden abgeglichen" + nic + " Schnittstelle."; }
            if (handle == 4294967290) { return "Der gefälschte Paketfilter wurde angepasst" + nic + " Schnittstelle."; }
            return "Filter" + handle + " wurde auf abgestimmt" + nic + " Schnittstelle.";
        }

        if (eventSensorType == 192) {
            if (eventDataField[2] == 0) return "Sicherheitsrichtlinie aufgerufen. Ein Teil oder der gesamte Netzwerkverkehr (TX) wurde gestoppt.";
            if (eventDataField[2] == 2) return "Sicherheitsrichtlinie aufgerufen. Ein Teil oder der gesamte Netzwerkverkehr (RX) wurde gestoppt.";
            return "Sicherheitsrichtlinie aufgerufen.";
        }

        if (eventSensorType == 193) {
            if ((eventDataField[0] == 0xAA) && (eventDataField[1] == 0x30) && (eventDataField[2] == 0x00) && (eventDataField[3] == 0x00)) { return "Benutzeranforderung für Remoteverbindung."; }
            if ((eventDataField[0] == 0xAA) && (eventDataField[1] == 0x20) && (eventDataField[2] == 0x03) && (eventDataField[3] == 0x01)) { return "EAC-Fehler: Versuchen Sie, die Haltung zu ermitteln, während NAC in Intel AMT deaktiviert ist."; } // eventDataField = 0xAA20030100000000
            if ((eventDataField[0] == 0xAA) && (eventDataField[1] == 0x20) && (eventDataField[2] == 0x04) && (eventDataField[3] == 0x00)) { return "HWA-Fehler: Allgemeiner Fehler"; } // Used to be "Certificate revoked." but don"t know the source of this.
        }

        if (eventSensorType == 6) return "Authentifizierung fehlgeschlagen" + (eventDataField[1] + (eventDataField[2] << 8)) + " mal. Das System wird möglicherweise angegriffen.";
        if (eventSensorType == 30) return "Keine bootfähigen Medien";
        if (eventSensorType == 32) return "Betriebssystemsperre oder Stromunterbrechung";
        if (eventSensorType == 35) return "Systemstartfehler";
        if (eventSensorType == 37) return "Die Systemfirmware wurde gestartet (mindestens eine CPU wird ordnungsgemäß ausgeführt).";
        return "Unbekannter Sensortyp #" + eventSensorType;
    }
...
// ###BEGIN###{AuditLog}
    ...
    {
        16: "Sicherheitsadministrator",
        17: "RCO",
        18: "Umleitungsmanager",
        19: "Firmware Update Manager",
        20: "Sicherheitsüberwachungsprotokoll",
        21: "Netzwerkzeit",
        22: "Netzwerkadministration",
        23: "Speicherverwaltung",
        24: "Event Manager",
        25: "Leistungsschalter-Manager",
        26: "Agent Presence Manager",
        27: "Drahtlose Konfiguration",
        28: "EAC",
        29: "KVM",
        30: "Benutzer-Opt-In-Ereignisse",
        32: "Bildschirmausblendung",
        33: "Watchdog-Ereignisse",
        1600: "Bereitstellung gestartet",
        1601: "Bereitstellung abgeschlossen",
        1602: "ACL-Eintrag hinzugefügt",
        1603: "ACL-Eintrag geändert",
        1604: "ACL-Eintrag entfernt",
        1605: "ACL-Zugriff mit ungültigen Anmeldeinformationen",
        1606: "ACL-Eingangsstatus",
        1607: "TLS-Status geändert",
        1608: "TLS Server Certificate Set",
        1609: "TLS-Serverzertifikat entfernen",
        1610: "TLS Trusted Root Certificate hinzugefügt",
        1611: "TLS Trusted Root Certificate entfernt",
        1612: "TLS Preshared Key Set",
        1613: "Kerberos-Einstellungen geändert",
        1614: "Kerberos-Hauptschlüssel geändert",
        1615: "Blitzverschleißzähler zurückgesetzt",
        1616: "Netzteil geändert",
        1617: "Stellen Sie den Realm-Authentifizierungsmodus ein",
        1618: "Aktualisieren Sie den Client auf den Admin-Steuerungsmodus",
        1619: "Unprovisioning gestartet",
        1700: "Power Up durchgeführt",
        1701: "Power Down durchgeführt",
        1702: "Aus- und Wiedereinschalten",
        1703: "Reset durchgeführt",
        1704: "Legen Sie die Startoptionen fest",
        1800: "IDER-Sitzung geöffnet",
        1801: "IDER-Sitzung geschlossen",
        1802: "IDER aktiviert",
        1803: "IDER deaktiviert",
        1804: "SoL-Sitzung eröffnet",
        1805: "SoL-Sitzung geschlossen",
        1806: "SoL aktiviert",
        1807: "SoL deaktiviert",
        1808: "KVM-Sitzung gestartet",
        1809: "KVM-Sitzung beendet",
        1810: "KVM aktiviert",
        1811: "KVM deaktiviert",
        1812: "Das VNC-Passwort ist dreimal fehlgeschlagen",
        1900: "Firmware aktualisiert",
        1901: "Firmware-Aktualisierung fehlgeschlagen",
        2000: "Sicherheitsüberwachungsprotokoll gelöscht",
        2001: "Sicherheitsüberprüfungsrichtlinie geändert",
        2002: "Sicherheitsüberwachungsprotokoll deaktiviert",
        2003: "Sicherheitsüberwachungsprotokoll aktiviert",
        2004: "Sicherheitsüberwachungsprotokoll exportiert",
        2005: "Sicherheitsüberwachungsprotokoll wiederhergestellt",
        2100: "Intel&reg; ME Zeiteinstellung",
        2200: "TCPIP-Parameter eingestellt",
        2201: "Hostname festgelegt",
        2202: "Domain Name Set",
        2203: "VLAN-Parameter eingestellt",
        2204: "Link Policy Set",
        2205: "IPv6-Parameter festgelegt",
        2300: "Globale Speicherattribute festgelegt",
        2301: "Speicher EACL geändert",
        2302: "Speicher FPACL geändert",
        2303: "Speicherschreibvorgang",
        2400: "Benachrichtigung abonniert",
        2401: "Benachrichtigung abbestellt",
        2402: "Ereignisprotokoll gelöscht",
        2403: "Ereignisprotokoll eingefroren",
        2500: "CB-Filter hinzugefügt",
        2501: "CB-Filter entfernt",
        2502: "CB-Richtlinie hinzugefügt",
        2503: "CB-Richtlinie entfernt",
        2504: "CB-Standardrichtliniensatz",
        2505: "CB Heuristik Optionsset",
        2506: "CB Heuristics State Cleared",
        2600: "Agent Watchdog hinzugefügt",
        2601: "Agent Watchdog entfernt",
        2602: "Agent Watchdog-Aktionssatz",
        2700: "Drahtloses Profil hinzugefügt",
        2701: "Drahtloses Profil entfernt",
        2702: "Drahtloses Profil aktualisiert",
        2800: "EAC Posture Signer SET",
        2801: "EAC aktiviert",
        2802: "EAC deaktiviert",
        2803: "EAC Posture State",
        2804: "EAC-Set-Optionen",
        2900: "KVM-Anmeldung aktiviert",
        2901: "KVM-Anmeldung deaktiviert",
        2902: "KVM-Passwort geändert",
        2903: "KVM-Zustimmung erfolgreich",
        2904: "KVM-Zustimmung fehlgeschlagen",
        3000: "Änderung der Opt-In-Richtlinie",
        3001: "Einwilligungscode-Ereignis senden",
        3002: "Starten Sie das blockierte Opt-In-Ereignis"
    }
    ...
    obj.GetAuditLogExtendedDataStr = function (id, data) {
        ...
        if (id == 1605) { return ["Ungültiger ME-Zugriff", "Ungültiger MEBx-Zugriff"][data.charCodeAt(0)]; } // ACL Access with Invalid Credentials
        if (id == 1606) { var r = ["Behindert", "aktiviert"][data.charCodeAt(0)]; if (data.charCodeAt(1) == 0) { r += "," + data.substring(3); } return r;} // ACL Entry State
        if (id == 1607) { return "Fernbedienung" + ["NoAuth", "ServerAuth", "MutualAuth"][data.charCodeAt(0)] + ", Lokal" + ["NoAuth", "ServerAuth", "MutualAuth"][data.charCodeAt(1)]; } // TLS State Changed
        if (id == 1617) { return obj.RealmNames[ReadInt(data, 0)] + "," + ["NoAuth", "Auth", "Behindert"][data.charCodeAt(4)]; } // Set Realm Authentication Mode
        if (id == 1619) { return ["BIOS", "MEBx", "Lokale MEI", "Lokaler WSMAN", "Remote-WSAMN"][data.charCodeAt(0)]; } // Intel AMT Unprovisioning Started
        if (id == 1900) { return "Von" + ReadShort(data, 0) + '.' + ReadShort(data, 2) + '.' + ReadShort(data, 4) + '.' + ReadShort(data, 6) + " zu" + ReadShort(data, 8) + '.' + ReadShort(data, 10) + '.' + ReadShort(data, 12) + '.' + ReadShort(data, 14); } // Firmware Updated
        ...
        if (id == 3000) { return "Von" + ["Keiner", "KVM", "Alle"][data.charCodeAt(0)] + " zu" + ["Keiner", "KVM", "Alle"][data.charCodeAt(1)]; } // Opt-In Policy Change
        if (id == 3001) { return ["Erfolg", "3 mal fehlgeschlagen"][data.charCodeAt(0)]; } // Send Consent Code Event
        return null;
    }
        ...
        if (responses.Body['RecordsReturned'] > 0) {
            ...
            for (i in responses.Body['EventRecords']) {
                ...
                if (x['InitiatorType'] == 2) {
                    // Local
                    x['Initiator'] = '<i>' + "Lokal" + '</i>';
                    ptr = 5;
                }
                if (x['InitiatorType'] == 3) {
                    // KVM Default Port
                    x['Initiator'] = '<i>' + "KVM-Standardport" + '</i>';
                    ptr = 5;
                }
