...
function AmtStackCreateService(wsmanStack) {
    ...
    var _EventTrapSourceTypes = "プラットフォームファームウェア（BIOSなど）| SMIハンドラー| ISVシステム管理ソフトウェア|アラートASIC | IPMI | BIOSベンダー|システムボードセットベンダー|システムインテグレーター|サードパーティアドイン| OSV | NIC |システム管理カード".split('|');
    ...
    function _GetEventDetailStr(eventSensorType, eventOffset, eventDataField, entity) {
        ...
        if (eventSensorType == 192) {
            if (eventDataField[2] == 0) return "呼び出されたセキュリティポリシー。一部またはすべてのネットワークトラフィック（TX）が停止しました。";
            if (eventDataField[2] == 2) return "呼び出されたセキュリティポリシー。一部またはすべてのネットワークトラフィック（RX）が停止しました。";
        ...
        if (eventSensorType == 193) {
            ...
            if ((eventDataField[0] == 0xAA) && (eventDataField[1] == 0x20) && (eventDataField[2] == 0x03) && (eventDataField[3] == 0x01)) { return "EACエラー：Intel AMTのNACが無効になっているときにポスチャを取得しようとしました。"; } // eventDataField = 0xAA20030100000000
            if ((eventDataField[0] == 0xAA) && (eventDataField[1] == 0x20) && (eventDataField[2] == 0x04) && (eventDataField[3] == 0x00)) { return "HWAエラー：一般的なエラー"; } // Used to be "Certificate revoked." but don"t know the source of this.
        }
        ...
        if (eventSensorType == 35) return "システムの起動失敗";
        if (eventSensorType == 37) return "システムファームウェアが起動しました（少なくとも1つのCPUが正しく実行されています）。";
