...
function AmtStackCreateService(wsmanStack) {
    ...
    obj.RealmNames = "|| Redirection || Hardware Asset | Remote Control | Storage | Event Manager | Storage Admin | Agent Presence Local | Agent Presence Remote | Circuit Breaker | Network Time | General Information | Firmware Update | EIT | LocalUN | Endpoint Access Control | Endpoint Access Control Beheerder | Gebeurtenislogboeklezer | Auditlogboek | ACL-domein ||| Lokaal systeem".split('|');
    obj.WatchdogCurrentStates = { 1: "Niet begonnen", 2: "Gestopt", 4: "Rennen", 8: "Niet meer geldig", 16: "Geschorst" };
    ...
    function _GetEventDetailStr(eventSensorType, eventOffset, eventDataField, entity) {
        if (eventSensorType == 15) {
            if (eventDataField[0] == 235) return "Onjuiste data";
            if (eventOffset == 0) {
        ...
        if ((eventSensorType == 18) && (eventDataField[0] == 170)) { // System watchdog event
            return "Agent waakhond" + char2hex(eventDataField[4]) + char2hex(eventDataField[3]) + char2hex(eventDataField[2]) + char2hex(eventDataField[1]) + '-' + char2hex(eventDataField[6]) + char2hex(eventDataField[5]) + '-...' + " veranderd in" + obj.WatchdogCurrentStates[eventDataField[7]];
        }
        ...
        {
            if (eventDataField[2] == 0) return "Er is een externe Serial Over LAN-sessie tot stand gebracht.";
            if (eventDataField[2] == 1) return "Remote Serial Over LAN-sessie beëindigd. Gebruikerscontrole is hersteld.";
            if (eventDataField[2] == 2) return "Er is een externe IDE-omleidingssessie opgezet.";
            if (eventDataField[2] == 3) return "Remote IDE-Redirection-sessie voltooid. Gebruikerscontrole is hersteld.";
        }
        ...
        if (eventSensorType == 36) {
            ...
            if (eventDataField[0] == 0xAA) nic = "bedrade"; // TODO: Add wireless *****
            //if (eventDataField[0] == 0xAA) nic = "wireless";

            if (handle == 4294967293) { return "Alle ontvangen pakketfilter is gematcht" + nic + " koppel."; }
            if (handle == 4294967292) { return "Alle uitgaande pakketfilters zijn gematcht" + nic + " koppel."; }
            if (handle == 4294967290) { return "Er werd op een vervalst pakketfilter gezocht" + nic + " koppel."; }
            return "Filter" + handle + " werd gematcht op" + nic + " koppel.";
        }

        if (eventSensorType == 192) {
            if (eventDataField[2] == 0) return "Beveiligingsbeleid ingeroepen. Een deel van of al het netwerkverkeer (TX) is gestopt.";
            if (eventDataField[2] == 2) return "Beveiligingsbeleid ingeroepen. Sommige of al het netwerkverkeer (RX) werd gestopt.";
            return "Beveiligingsbeleid ingeroepen.";
        }

        if (eventSensorType == 193) {
            if ((eventDataField[0] == 0xAA) && (eventDataField[1] == 0x30) && (eventDataField[2] == 0x00) && (eventDataField[3] == 0x00)) { return "Gebruikersverzoek voor externe verbinding."; }
            if ((eventDataField[0] == 0xAA) && (eventDataField[1] == 0x20) && (eventDataField[2] == 0x03) && (eventDataField[3] == 0x01)) { return "EAC-fout: probeer houding te krijgen terwijl NAC in Intel� AMT is uitgeschakeld."; } // eventDataField = 0xAA20030100000000
            if ((eventDataField[0] == 0xAA) && (eventDataField[1] == 0x20) && (eventDataField[2] == 0x04) && (eventDataField[3] == 0x00)) { return "HWA-fout: algemene fout"; } // Used to be "Certificate revoked." but don"t know the source of this.
        }

        if (eventSensorType == 6) return "Verificatie mislukt" + (eventDataField[1] + (eventDataField[2] << 8)) + " keer. Het systeem wordt mogelijk aangevallen.";
        if (eventSensorType == 30) return "Geen opstartbare media";
        if (eventSensorType == 32) return "Vergrendeling van besturingssysteem of stroomonderbreking";
        if (eventSensorType == 35) return "Systeem opstartfout";
        if (eventSensorType == 37) return "Systeemfirmware is gestart (ten minste één CPU wordt correct uitgevoerd).";
        return "Onbekend sensortype #" + eventSensorType;
    }
...
// ###BEGIN###{AuditLog}
    ...
    {
        16: "Beveiligingsbeheerder",
        17: "RCO",
        18: "Redirection Manager",
        19: "Firmware Update Manager",
        20: "Beveiligingsauditlogboek",
        21: "Netwerktijd",
        22: "Netwerk administratie",
        23: "Opslagbeheer",
        24: "Event Manager",
        25: "Circuit Breaker Manager",
        26: "Agent Presence Manager",
        27: "Draadloze configuratie",
        28: "EAC",
        29: "KVM",
        30: "Gebruikersaanmeldingsgebeurtenissen",
        32: "Scherm leegmaken",
        33: "Watchdog-evenementen",
        1600: "Bevoorrading gestart",
        1601: "Provisioning voltooid",
        1602: "ACL-vermelding toegevoegd",
        1603: "ACL-vermelding gewijzigd",
        1604: "ACL-vermelding verwijderd",
        1605: "ACL-toegang met ongeldige referenties",
        1606: "ACL-status van binnenkomst",
        1607: "TLS-status gewijzigd",
        1608: "TLS-servercertificaatset",
        1609: "TLS-servercertificaat verwijderen",
        1610: "TLS Trusted Root Certificate toegevoegd",
        1611: "TLS Trusted Root Certificate verwijderd",
        1612: "TLS vooraf gedeelde sleutelset",
        1613: "Kerberos-instellingen gewijzigd",
        1614: "Kerberos hoofdsleutel gewijzigd",
        1615: "Flash Versleten tellers Reset",
        1616: "Power Package gewijzigd",
        1617: "Stel Realm Authentication Mode in",
        1618: "Upgrade Client naar Admin Control Mode",
        1619: "Unprovisioning Begonnen",
        1700: "Power Up uitgevoerd",
        1701: "Uitschakelen uitgevoerd",
        1702: "Power Cycle uitgevoerd",
        1703: "Reset uitgevoerd",
        1704: "Stel opstartopties in",
        1800: "IDER-sessie geopend",
        1801: "IDER-sessie gesloten",
        1802: "IDER ingeschakeld",
        1803: "IDER uitgeschakeld",
        1804: "SoL-sessie geopend",
        1805: "SoL-sessie gesloten",
        1806: "SoL ingeschakeld",
        1807: "SoL uitgeschakeld",
        1808: "KVM-sessie gestart",
        1809: "KVM-sessie beëindigd",
        1810: "KVM ingeschakeld",
        1811: "KVM uitgeschakeld",
        1812: "VNC-wachtwoord 3 keer mislukt",
        1900: "Firmware bijgewerkt",
        1901: "Firmware-update mislukt",
        2000: "Beveiligingsauditlogboek gewist",
        2001: "Beveiligingsauditbeleid gewijzigd",
        2002: "Beveiligingsauditlogboek uitgeschakeld",
        2003: "Beveiligingsauditlogboek ingeschakeld",
        2004: "Beveiligingsauditlogboek geëxporteerd",
        2005: "Beveiligingsauditlogboek hersteld",
        2100: "Intel&reg; ME Time Set",
        2200: "TCPIP-parameters ingesteld",
        2201: "Hostnaam ingesteld",
        2202: "Domeinnaam ingesteld",
        2203: "VLAN-parameterset",
        2204: "Linkbeleid instellen",
        2205: "IPv6-parameters ingesteld",
        2300: "Global Storage Attributes Set",
        2301: "Opslag EACL gewijzigd",
        2302: "Opslag FPACL gewijzigd",
        2303: "Opslag schrijven",
        2400: "Waarschuwing geabonneerd",
        2401: "Melding afgemeld",
        2402: "Gebeurtenislogboek gewist",
        2403: "Gebeurtenislogboek bevroren",
        2500: "CB-filter toegevoegd",
        2501: "CB-filter verwijderd",
        2502: "CB-beleid toegevoegd",
        2503: "CB-beleid verwijderd",
        2504: "CB Standaardbeleid ingesteld",
        2505: "CB Heuristics Option Set",
        2506: "CB Heuristics State Cleared",
        2600: "Agent Watchdog toegevoegd",
        2601: "Agent Watchdog verwijderd",
        2602: "Agent Watchdog-actieset",
        2700: "Draadloos profiel toegevoegd",
        2701: "Draadloos profiel verwijderd",
        2702: "Draadloos profiel bijgewerkt",
        2800: "EAC Posture Signer SET",
        2801: "EAC ingeschakeld",
        2802: "EAC uitgeschakeld",
        2803: "EAC Posture State",
        2804: "EAC-opties instellen",
        2900: "KVM-aanmelding ingeschakeld",
        2901: "KVM-aanmelding uitgeschakeld",
        2902: "KVM-wachtwoord gewijzigd",
        2903: "KVM-toestemming geslaagd",
        2904: "KVM-toestemming mislukt",
        3000: "Opt-in beleidswijziging",
        3001: "Toestemmingscode-gebeurtenis verzenden",
        3002: "Start opt-in geblokkeerde gebeurtenis"
    }
    ...
    obj.GetAuditLogExtendedDataStr = function (id, data) {
        ...
        if (id == 1605) { return ["Ongeldige ME-toegang", "Ongeldige MEBx-toegang"][data.charCodeAt(0)]; } // ACL Access with Invalid Credentials
        if (id == 1606) { var r = ["Gehandicapt", "Ingeschakeld"][data.charCodeAt(0)]; if (data.charCodeAt(1) == 0) { r += "," + data.substring(3); } return r;} // ACL Entry State
        if (id == 1607) { return "Afgelegen" + ["NoAuth", "ServerAuth", "MutualAuth"][data.charCodeAt(0)] + ", Lokaal" + ["NoAuth", "ServerAuth", "MutualAuth"][data.charCodeAt(1)]; } // TLS State Changed
        if (id == 1617) { return obj.RealmNames[ReadInt(data, 0)] + "," + ["NoAuth", "Auth", "Gehandicapt"][data.charCodeAt(4)]; } // Set Realm Authentication Mode
        if (id == 1619) { return ["BIOS", "MEBx", "Lokale MEI", "Lokale WSMAN", "Externe WSAMN"][data.charCodeAt(0)]; } // Intel AMT Unprovisioning Started
        if (id == 1900) { return "Van" + ReadShort(data, 0) + '.' + ReadShort(data, 2) + '.' + ReadShort(data, 4) + '.' + ReadShort(data, 6) + " naar" + ReadShort(data, 8) + '.' + ReadShort(data, 10) + '.' + ReadShort(data, 12) + '.' + ReadShort(data, 14); } // Firmware Updated
        ...
        if (id == 3000) { return "Van" + ["Geen", "KVM", "Allemaal"][data.charCodeAt(0)] + " naar" + ["Geen", "KVM", "Allemaal"][data.charCodeAt(1)]; } // Opt-In Policy Change
        if (id == 3001) { return ["Succes", "3 keer mislukt"][data.charCodeAt(0)]; } // Send Consent Code Event
        return null;
    }
        ...
        if (responses.Body['RecordsReturned'] > 0) {
            ...
            for (i in responses.Body['EventRecords']) {
                ...
                if (x['InitiatorType'] == 2) {
                    // Local
                    x['Initiator'] = '<i>' + "Lokaal" + '</i>';
                    ptr = 5;
                }
                if (x['InitiatorType'] == 3) {
                    // KVM Default Port
                    x['Initiator'] = '<i>' + "KVM-standaardpoort" + '</i>';
                    ptr = 5;
                }
