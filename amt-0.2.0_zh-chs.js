...
function AmtStackCreateService(wsmanStack) {
    ...
    var _EventTrapSourceTypes = "平台固件（例如BIOS）| SMI处理程序| ISV系统管理软件|警报ASIC | IPMI | BIOS供应商|系统板集供应商|系统集成商|第三方加载项| OSV | NIC |系统管理卡".split('|');
    var _SystemFirmwareError = "未指定。|没有在系统中物理安装系统内存。|没有可用的系统内存，所有安装的内存都经历了不可恢复的故障。|不可恢复的硬盘/ ATAPI / IDE设备故障。|不可恢复的主板故障。|不可恢复的软盘子系统故障。|不可恢复的硬盘控制器故障。|不可恢复的PS / 2或USB键盘故障。|找不到可移动的启动媒体。|不可恢复的视频控制器故障。|未检测到视频设备。|检测到固件（BIOS）ROM损坏。 CPU电压不匹配（共享电源的处理器对电压的要求不匹配）| CPU速度匹配失败".split('|');
    var _SystemFirmwareProgress = "未指定。总线初始化|键盘控制器初始化|嵌入式控制器/管理控制器初始化|对接站附件|启用对接站|对接站弹出|禁用对接站|调用操作系统唤醒向量|启动操作系统启动过程|基板或主板初始化|已保留|软盘初始化|键盘测试|指针设备测试|主处理器初始化".split('|');
    var _SystemEntityTypes = "未指定|其他|未知|处理器|磁盘|外围设备|系统管理模块|系统板|内存模块|处理器模块|电源|添加卡|前面板板|后面板板|电源系统板|驱动器背板|系统内部扩展板|其他系统板|处理器板|电源单元|电源模块|电源管理板|机箱后面板板|系统机箱|子机箱|其他机箱板|磁盘驱动器托架|外围设备托架|设备托架|风扇冷却|冷却单元|电缆互连|内存设备|系统管理软件| BIOS | Intel（r）ME |系统总线|组| Intel（r）ME |外部环境|电池|处理刀片|连接开关|处理器/内存模块| I / O模块|处理器I / O模块|管理控制器固件| IPMI通道| PCI总线| PCI Express总线| SCSI总线| SATA / SAS总线|处理器前端总线".split('|');
    obj.RealmNames = "||重定向||硬件资产|远程控制|存储|事件管理器|存储管理|代理本地存在|代理远程|断路器|网络时间|一般信息|固件更新| EIT | LocalUN |端点访问控制|端点访问控制管理员|事件日志读取器|审计日志| ACL域|||本地系统".split('|');
    obj.WatchdogCurrentStates = { 1: "没有开始", 2: "已停止", 4: "跑步", 8: "已过期", 16: "已暂停" };
    ...
    function _GetEventDetailStr(eventSensorType, eventOffset, eventDataField, entity) {
        if (eventSensorType == 15) {
            if (eventDataField[0] == 235) return "无效数据";
            if (eventOffset == 0) {
        ...
        if ((eventSensorType == 18) && (eventDataField[0] == 170)) { // System watchdog event
            return "代理看门狗" + char2hex(eventDataField[4]) + char2hex(eventDataField[3]) + char2hex(eventDataField[2]) + char2hex(eventDataField[1]) + '-' + char2hex(eventDataField[6]) + char2hex(eventDataField[5]) + '-...' + " 变成" + obj.WatchdogCurrentStates[eventDataField[7]];
        }
        ...
        {
            if (eventDataField[2] == 0) return "建立了远程LAN上串行会话。";
            if (eventDataField[2] == 1) return "LAN远程串行会话已完成。用户控件已还原。";
            if (eventDataField[2] == 2) return "建立了远程IDE重定向会话。";
            if (eventDataField[2] == 3) return "远程IDE重定向会话已完成。用户控件已还原。";
        }
        ...
        if (eventSensorType == 36) {
            ...
            if (eventDataField[0] == 0xAA) nic = "有线"; // TODO: Add wireless *****
            //if (eventDataField[0] == 0xAA) nic = "wireless";

            if (handle == 4294967293) { return "所有接收到的数据包过滤器均已匹配" + nic + " 接口。"; }
            if (handle == 4294967292) { return "所有出站数据包过滤器均已匹配" + nic + " 接口。"; }
            if (handle == 4294967290) { return "欺骗性封包筛选器已匹配" + nic + " 接口。"; }
            return "过滤" + handle + " 被匹配" + nic + " 接口。";
        }

        if (eventSensorType == 192) {
            if (eventDataField[2] == 0) return "调用安全策略。某些或所有网络流量（TX）已停止。";
            if (eventDataField[2] == 2) return "调用安全策略。某些或所有网络流量（RX）已停止。";
            return "调用安全策略。";
        }

        if (eventSensorType == 193) {
            if ((eventDataField[0] == 0xAA) && (eventDataField[1] == 0x30) && (eventDataField[2] == 0x00) && (eventDataField[3] == 0x00)) { return "用户请求远程连接。"; }
            if ((eventDataField[0] == 0xAA) && (eventDataField[1] == 0x20) && (eventDataField[2] == 0x03) && (eventDataField[3] == 0x01)) { return "EAC错误：禁用Intel AMT中的NAC时尝试姿势。"; } // eventDataField = 0xAA20030100000000
            if ((eventDataField[0] == 0xAA) && (eventDataField[1] == 0x20) && (eventDataField[2] == 0x04) && (eventDataField[3] == 0x00)) { return "HWA错误：一般错误"; } // Used to be "Certificate revoked." but don"t know the source of this.
        }

        if (eventSensorType == 6) return "验证失败" + (eventDataField[1] + (eventDataField[2] << 8)) + " 次。系统可能受到攻击。";
        if (eventSensorType == 30) return "没有可启动媒体";
        if (eventSensorType == 32) return "操作系统锁定或电源中断";
        if (eventSensorType == 35) return "系统启动失败";
        if (eventSensorType == 37) return "系统固件已启动（至少一个CPU正在正确执行）。";
        return "未知传感器类型" + eventSensorType;
    }
...
// ###BEGIN###{AuditLog}
    ...
    obj.GetAuditLogExtendedDataStr = function (id, data) {
        ...
        if (id == 1605) { return ["无效的ME访问", "无效的MEBx访问"][data.charCodeAt(0)]; } // ACL Access with Invalid Credentials
        if (id == 1606) { var r = ["残障人士", "已启用"][data.charCodeAt(0)]; if (data.charCodeAt(1) == 0) { r += "，" + data.substring(3); } return r;} // ACL Entry State
        if (id == 1607) { return "远程" + ["无认证", "服务器认证", "相互认证"][data.charCodeAt(0)] + "，本地" + ["无认证", "服务器认证", "相互认证"][data.charCodeAt(1)]; } // TLS State Changed
        if (id == 1617) { return obj.RealmNames[ReadInt(data, 0)] + "，" + ["无认证", "验证码", "残障人士"][data.charCodeAt(4)]; } // Set Realm Authentication Mode
        if (id == 1619) { return ["的BIOS", "城域网", "本地MEI", "本地WSMAN", "远程WSAMN"][data.charCodeAt(0)]; } // Intel AMT Unprovisioning Started
        if (id == 1900) { return "从" + ReadShort(data, 0) + '.' + ReadShort(data, 2) + '.' + ReadShort(data, 4) + '.' + ReadShort(data, 6) + " 至" + ReadShort(data, 8) + '.' + ReadShort(data, 10) + '.' + ReadShort(data, 12) + '.' + ReadShort(data, 14); } // Firmware Updated
        ...
        if (id == 3000) { return "从" + ["没有", "虚拟机", "所有"][data.charCodeAt(0)] + " 至" + ["没有", "虚拟机", "所有"][data.charCodeAt(1)]; } // Opt-In Policy Change
        if (id == 3001) { return ["成功", "失败3次"][data.charCodeAt(0)]; } // Send Consent Code Event
        return null;
    }
